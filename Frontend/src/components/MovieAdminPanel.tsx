...
const MovieAdminPanel: React.FC = () => {
  ...
  // lazy import helper for static/live mode
  ...
  const handleCreateMovie = async (e: React.FormEvent) => {
    ...
    try {
      if (USE_STATIC) {
        const created: Movie = { ...newMovie, IdMovie: Date.now() };
  ...
  const handleUpdateMovie = async (e: React.FormEvent) => {
    ...
    try {
      if (USE_STATIC) {
        setMovies(
          movies.map((m) =>
            m.IdMovie === editingMovie.IdMovie ? editingMovie : m
          )
        );
  ...
  const generateReport = (filtered: boolean = false) => {
    ...
    link.setAttribute(
      "download",
      `reporte_peliculas_${filtered ? "filtrado" : "completo"}_${
