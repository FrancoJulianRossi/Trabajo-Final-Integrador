...
import type { Seat } from "./AdminPanelRoom/types"; // Re-using the central type
...
export const SeleccionAsientos: React.FC<{
  ...
  const getSeatState = (seat: Seat) => {
    if (occupied.some((o) => o.row === seat.row && o.column === seat.column))
      return "Occupied";
  ...
  if (loading) {
    return (
      <Container className="py-5 text-center">Cargando asientos...</Container>
    );
  }
    ...
    return (
      <Container className="py-5">
        <p className="text-center text-danger">
          No hay mapa de asientos para esta sala.
        </p>
      ...
      <Row className="justify-content-center">
        <Col md={10} lg={8}>
          ...
          <div className="mt-4 mb-4 p-3 border rounded bg-light">
            <div className="mb-4 w-75 mx-auto text-center p-1 bg-dark text-white rounded">
              PANTALLA
            </div>
            {renderGrid()}
          </div>
          ...
          <div className="d-flex justify-content-center flex-wrap gap-3 mb-4">
            {Object.entries(SEAT_COLORS).map(
              ([type, color]) =>
                type !== "Empty" && (
