...
export const AdminPanelRoom: React.FC = () => {
  ...
  async function handleDelete(r: Room) {
    ...
    try {
      if (USE_STATIC) {
        setRooms((p) => p.filter((it) => it.idRoom !== r.idRoom));
        return;
      }
  ...
  return (
    <Container className="py-4">
      <Row className="mb-3 align-items-center">
        <Col>
          ...
          <div className="text-muted">
            Listado, búsqueda, filtros por capacidad, edición, eliminación y
            reportes
          </div>
        </Col>
      ...
      <Row className="mb-2">
        <Col md={4}>
          <Form.Control
            placeholder="Buscar por id/nombre/ubicación/layout..."
            value={search}
        ...
        <Col md={3}>
          ...
          >
            ...
            <option value="medium">
              Medianas 51-100 ({capacityCounts.medium})
            </option>
            <option value="large">
              Grandes &gt;100 ({capacityCounts.large})
            </option>
          </Form.Select>
        </Col>
        <Col md={2}>
      ...
      <Modal show={!!editing} onHide={() => setEditing(null)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            {editing ? `Editar sala #${editing.idRoom}` : "Editar"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {editing && (
